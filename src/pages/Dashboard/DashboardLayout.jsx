...
// DashboardLayout.jsx
// Route: /dashboard  (non-admin variant — same features, different base path)
// Handles: /dashboard, /dashboard/create, /dashboard/edit/:id
...
// ─── Category config ──────────────────────────────────────────────────────────
// Shared with AdminDashboard — ideally import from a shared constants file.
...
export default function DashboardLayout() {
  ...
  useEffect(() => {
    const path      = location.pathname;
    const editMatch = path.match(/\/dashboard\/edit\/([^/]+)/);
  ...
  const goToEdit = (id) => {
    if (!id) {
      console.error('DashboardLayout: goToEdit called without a post id');
      return;
    }
    setEditPostId(id);
    setCurrentView('edit');
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case 'create':
        return <CreatePost onSuccess={goToPosts} />;
      case 'edit':
        return editPostId ? (
          <EditPost postId={editPostId} onSuccess={goToPosts} />
        ) : (
      ...
      <div className="min-h-screen bg-gray-100">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between h-16">
              ...
              <div className="flex items-center space-x-3">
                <h1 className="text-xl font-bold text-gray-900 mr-2">Admin Dashboard</h1>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4">
          <p className="text-sm text-gray-500">
            <span className="hover:underline cursor-pointer" onClick={goToPosts}>
              Dashboard
            </span>
            {currentView !== 'posts' && (
              <>
